...
// Common wave animation style to match index.html exactly
const waveSx = {
    '&::after': {
        background: 'linear-gradient(90deg, transparent, rgba(255, 255, 255, 0.08), transparent)',
        animation: 'loading-shimmer 2s infinite linear', // Match HTML loader animation exactly
    }
};
...
// Hero skeleton - exactly matching the HTML dark gradient loader
...
// Navbar skeleton - overlays the hero
...
export const FooterSkeleton = () => (
    <Box sx={{ bgcolor: '#0D2B14', py: 8, borderTop: '1px solid rgba(255,255,255,0.05)' }}>
        <Container maxWidth="lg">
            <Grid container spacing={8}>
                <Grid item xs={12} md={4}>
                    <Skeleton variant="rounded" width={140} height={40} animation="wave" sx={{ bgcolor: 'rgba(255, 255, 255, 0.1)', mb: 3, borderRadius: 2, ...waveSx }} />
                    <Skeleton variant="text" width="80%" height={20} animation="wave" sx={{ bgcolor: 'rgba(255, 255, 255, 0.05)', mb: 1, ...waveSx }} />
                    <Skeleton variant="text" width="60%" height={20} animation="wave" sx={{ bgcolor: 'rgba(255, 255, 255, 0.05)', ...waveSx }} />
                </Grid>
                <Grid item xs={12} md={8}>
                    <Grid container spacing={4}>
                        {[1, 2, 3].map((col) => (
                            <Grid item xs={6} md={4} key={col}>
                                <Skeleton variant="text" width={100} height={24} animation="wave" sx={{ bgcolor: 'rgba(255, 255, 255, 0.1)', mb: 3, ...waveSx }} />
                                {[1, 2, 3, 4].map((item) => (
                                    <Skeleton key={item} variant="text" width="100%" height={20} animation="wave" sx={{ bgcolor: 'rgba(255, 255, 255, 0.05)', mb: 1.5, ...waveSx }} />
                                ))}
                            </Grid>
                        ))}
                    </Grid>
            ...
            <Box sx={{ mt: 8, pt: 4, borderTop: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 2 }}>
                <Skeleton variant="text" width={200} height={20} animation="wave" sx={{ bgcolor: 'rgba(255, 255, 255, 0.05)', ...waveSx }} />
                <Box sx={{ display: 'flex', gap: 2 }}>
                    {[1, 2, 3].map((i) => (
                        <Skeleton key={i} variant="circular" width={32} height={32} animation="wave" sx={{ bgcolor: 'rgba(255, 255, 255, 0.05)', ...waveSx }} />
                    ))}
                </Box>
            </Box>
        </Container>
...
export const PageSkeleton = () => {
    useEffect(() => {
        // Unlock body scroll which was hidden by index.html loader
    ...
    return (
        <Box>
            ...
            <Container maxWidth="lg" sx={{ py: 8 }}>
                <Skeleton variant="rounded" width="30%" height={48} animation="wave" sx={{ mb: 2, mx: 'auto', display: 'block', borderRadius: '8px', ...waveSx }} />
                <Skeleton variant="rounded" width="60%" height={24} animation="wave" sx={{ mb: 6, mx: 'auto', display: 'block', borderRadius: '4px', ...waveSx }} />
                <Grid container spacing={4}>
                    {[1, 2, 3].map((i) => (
...
export const GallerySkeleton = () => (
    <Container maxWidth="lg" sx={{ py: 8 }}>
        <Skeleton variant="rounded" width="25%" height={48} animation="wave" sx={{ mb: 4, mx: 'auto', display: 'block', borderRadius: '8px', ...waveSx }} />
        <Grid container spacing={2}>
            {[320, 240, 280, 200, 340, 260, 300, 220, 280].map((h, i) => (
                <Grid item xs={12} sm={6} md={4} key={i}>
                    <Skeleton variant="rounded" width="100%" height={h} animation="wave" sx={{ borderRadius: '12px', ...waveSx }} />
                </Grid>
            ))}
        </Grid>
    </Container>
